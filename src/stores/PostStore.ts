...
function generateTestPosts(count: number) : Array<IPost> {
...
export const usePostStore = defineStore("postStore", {
	state: () => ({
		...
		apiTimestamps: Array.from({ length: 100 }, () => new Date()) as Date[]
	}),

	actions: {
		async postExists(id: string): Promise<boolean> {
			if (this.posts.find((p) => p.id == id) !== undefined) return true;
			// TODO: call API
			return false;
		},
		async getPostById(id: string): Promise<IPost> {
			...
			// TODO: Api call
			return this.posts.find(p=>p.id==id) as IPost;
		}
	},
